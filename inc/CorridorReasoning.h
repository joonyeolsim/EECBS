...
class CorridorReasoning {
 public:
  ...
  shared_ptr<Conflict> run(const shared_ptr<Conflict>& conflict, const vector<Path*>& paths,
                           const HLNode& node);
 ...
 private:
  ...
  // num_col, int map_size); int getBypassLengthByAStar(int start, int end, std::pair<int, int>
  // blocked,

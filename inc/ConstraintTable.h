...
class ConstraintTable {
 public:
  ...
  int getHoldingTime(int location, int earliest_timestep)
      const;  // the earliest timestep that the agent can hold the location after earliest_timestep
  int getMaxTimestep() const;  // everything is static after the max timestep
  int getLastCollisionTimestep(int location) const;
  // void clear(){ct.clear(); cat_small.clear(); cat_large.clear(); landmarks.clear(); length_min =
  // 0, length_max = INT_MAX; latest_timestep = 0;}
  ...
  void insert2CT(const HLNode& node,
                 int agent);  // build the constraint table for the given agent at the give node
 ...
 protected:
  ...
  void insertLandmark(size_t loc, int t);  // insert a landmark, i.e., the agent has to be at the
                                           // given location at the given timestep

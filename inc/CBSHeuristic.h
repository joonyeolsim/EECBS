      ...
      while (curr->parent != nullptr) {
        if (get<4>(curr->constraints.front()) == constraint_type::LEQLENGTH ||
          ...
          if (get<0>(curr->constraints.front()) == h1.a1)
            for (auto con : curr->constraints) cons1[0].insert(con);
          else if (get<0>(curr->constraints.front()) == h1.a2)
            for (auto con : curr->constraints) cons2[0].insert(con);
        }
      ...
      while (curr->parent != nullptr) {
        if (get<4>(curr->constraints.front()) == constraint_type::LEQLENGTH ||
          ...
          if (get<0>(curr->constraints.front()) == h2.a1)
            for (auto con : curr->constraints) cons1[1].insert(con);
          else if (get<0>(curr->constraints.front()) == h2.a2)
            for (auto con : curr->constraints) cons2[1].insert(con);
        }

        curr = curr->parent;
      }
      if (cons1[0].size() != cons1[1].size() || cons2[0].size() != cons2[1].size()) return false;

      if (!equal(cons1[0].begin(), cons1[0].end(), cons1[1].begin())) return false;
    ...
    size_t operator()(const HTableEntry& entry) const {
      ...
      while (curr->parent != nullptr) {
        if (get<0>(curr->constraints.front()) == entry.a1 ||
          ...
          for (auto con : curr->constraints) {
            cons1_hash +=
                3 * std::hash<int>()(std::get<0>(con)) + 5 * std::hash<int>()(std::get<1>(con)) +
                7 * std::hash<int>()(std::get<2>(con)) + 11 * std::hash<int>()(std::get<3>(con));
          }
...
typedef unordered_map<HTableEntry, tuple<int, int, int>, HTableEntry::Hasher, HTableEntry::EqNode>
    HTable;
...
class CBSHeuristic {
 public:
  ...
  // stats
  list<tuple<int, int, const HLNode*, uint64_t, int> >
      sub_instances;  // <agent 1, agent 2, node, number of expanded CT nodes, h value>

  CBSHeuristic(int num_of_agents, const vector<Path*>& paths,
  ...
  void setInadmissibleHeuristics(heuristics_type h) {
    ...
    if (h == heuristics_type::CONFLICT) {
      sum_distance_errors.assign(
          int(conflict_type::TYPE_COUNT),
 ...
 private:
  ...
  double time_limit;
  int node_limit = 4;  // terminate the sub CBS solver if the number of its expanded nodes exceeds
                       // the node limit.
  double start_time;
  int ILP_node_threshold =
      5;  // when #nodes >= ILP_node_threshold, use ILP solver; otherwise, use DP solver
  int ILP_edge_threshold =
      10;  // when #edges >= ILP_edge_threshold, use ILP solver; otherwise, use DP solver
  int ILP_value_threshold =
      32;  // when value >= ILP_value_threshold, use ILP solver; otherwise, use DP solver
  ...
  bool buildWeightedDependencyGraph(ECBSNode& node, const vector<int>& min_f_vals, vector<int>& CG,
                                    int& delta_g);
  bool dependent(int a1, int a2, HLNode& node);  // return true if the two agents are dependent
  pair<int, int> solve2Agents(int a1, int a2, const CBSNode& node,
                              bool cardinal);  // return h value and num of CT nodes
  tuple<int, int, int> solve2Agents(int a1, int a2,
                                    const ECBSNode& node);  // return h value and num of CT nodes
  static bool SyncMDDs(const MDD& mdd1,
                       const MDD& mdd2);  // Match and prune MDD according to another MDD.
  // void setUpSubSolver(CBS& cbs) const;
  int minimumVertexCover(const vector<int>& CG);  // mvc on disjoint components
  int minimumVertexCover(const vector<int>& CG, int old_mvc, int cols,
                         int num_of_edges);  // incremental mvc
  ...
  // int ILPForWMVC(const vector<int>& CG, const vector<int>& range) const; // Integer linear
  // programming
  int ILPForConstrainedWMVC(const std::vector<int>& CG, const std::vector<int>& range);
  int DPForConstrainedWMVC(vector<bool>& x, int i, int sum, const vector<int>& CG,
                           const vector<int>& range, int& best_so_far);
};

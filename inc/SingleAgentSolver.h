...
class LLNode  // low-level node
{
 public:
  ...
  bool wait_at_goal;  // the action is to wait at the goal vertex or not. This is used for >length
                      // constraints
  bool is_goal = false;
    ...
    bool operator()(const LLNode* n1, const LLNode* n2) const  // returns true if n1 > n2
    {
      if (n1->num_of_conflicts == n2->num_of_conflicts) {
        ...
        return n1->g_val + n1->h_val >=
               n2->g_val +
                   n2->h_val;  // break ties towards smaller f_vals (prefer shorter solutions)
      }
...
class SingleAgentSolver {
 public:
  ...
  virtual pair<Path, int> findSuboptimalPath(const HLNode& node,

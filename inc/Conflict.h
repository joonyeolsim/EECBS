...
// <agent, loc, -1, t, LEQLENGTH>: path of agent_id should be of length at most t, and any other
// agent cannot be at loc at or after timestep t <agent, loc, -1, t, GLENGTH>: path of agent_id
// should be of length at least t + 1
...
class Conflict {
 public:
  ...
  int getConflictId() const {
    return int(type);
  }  // int(PRIORITY_COUNT) * int(type) + int(priority); }

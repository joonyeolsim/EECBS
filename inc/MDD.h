...
class MDD {
 ...
 public:
  ...
  // bool buildMDD(const std::vector <std::list< std::pair<int, int> > >& constraints, int
  // numOfLevels, 	int start_location, const int* moves_offset, const std::vector<int>& my_heuristic,
  // int map_size, int num_col);

  MDDNode* find(int location, int level) const;
  void deleteNode(MDDNode* node);
  void clear();
  // bool isConstrained(int curr_id, int next_id, int next_timestep, const std::vector< std::list<
...
class SyncMDDNode {
 public:
  ...
  bool operator==(const SyncMDDNode& node) const { return (this->location == node.location); }
...
class MDDTable {
 public:
  ...
  void init(int number_of_agents) { lookupTable.resize(number_of_agents); }
  ~MDDTable() { clear(); }
 ...
 private:
  ...
  vector<
      unordered_map<ConstraintsHasher, MDD*, ConstraintsHasher::Hasher, ConstraintsHasher::EqNode>>
      lookupTable;

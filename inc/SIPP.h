...
class SIPPNode : public LLNode {
 public:
  ...
  bool collision_v;
  SIPPNode() : LLNode() {}
  SIPPNode(int loc, int g_val, int h_val, SIPPNode* parent, int timestep, int high_generation,
  ...
  // SIPPNode(const SIPPNode& other): LLNode(other), high_generation(other.high_generation),
  ...
  struct eqnode {
    bool operator()(const SIPPNode* n1, const SIPPNode* n2) const {
      return (n1 == n2) ||
             (n1 && n2 && n1->location == n2->location && n1->wait_at_goal == n2->wait_at_goal &&
              n1->is_goal == n2->is_goal && n1->high_generation == n2->high_generation);
    }
  };
};
...
class SIPP : public SingleAgentSolver {
 public:
  ...
  // minimizing the number of internal conflicts (that is conflicts with known_paths for other
  // agents found so far). lowerbound is an underestimation of the length of the path in order to
  // speed up the search.
  ...
  Path findPath(const ConstraintTable& constraint_table);  // return A path that minimizes
 ...
 private:
  ...
  typedef boost::heap::pairing_heap<SIPPNode*, boost::heap::compare<LLNode::compare_node> >
      heap_open_t;
  typedef boost::heap::pairing_heap<SIPPNode*,
                                    boost::heap::compare<LLNode::secondary_compare_node> >
      heap_focal_t;

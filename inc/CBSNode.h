...
class CBSNode : public HLNode {
 public:
  ...
  pairing_heap<CBSNode*, compare<CBSNode::compare_node_by_inadmissible_f> >::handle_type
      open_handle;
    ...
    bool operator()(const ConstraintsHasher& c1, const ConstraintsHasher& c2) const {
      ...
      while (curr->parent != nullptr) {
        if (get<4>(curr->constraints.front()) == constraint_type::LEQLENGTH ||
          ...
          for (auto con : curr->constraints) cons1.insert(con);
        }
        curr = curr->parent;
      }
      curr = c2.n;
      while (curr->parent != nullptr) {
        if (get<4>(curr->constraints.front()) == constraint_type::LEQLENGTH ||
          ...
          for (auto con : curr->constraints) cons2.insert(con);
        }

...
int main(int argc, char** argv) {
  ...
  desc.add_options()("help", "produce help message")
      ...
      ("map,m", po::value<string>()->required(), "input file for map")(
          ...
          "screen option (0: none; 1: results; 2:all)")("stats",
      ...
      ("highLevelSolver", po::value<string>()->default_value("EES"),
       "the high-level solver (A*, A*eps, EES, NEW)")("lowLevelSolver",
  ...
  if (s == high_level_solver_type::ASTAR || s == high_level_solver_type::ASTAREPS ||
  ...
  if (vm["lowLevelSolver"].as<bool>()) {
    ...
    if (vm.count("output")) ecbs.saveResults(vm["output"].as<string>(), vm["agents"].as<string>());
    ...
    string output_name = vm["output"].as<string>().substr(0, pos);     // get the name without
    ...
    cbs.runtime = runtime;
    if (vm.count("output")) cbs.saveResults(vm["output"].as<string>(), vm["agents"].as<string>());
    if (cbs.solution_found && vm.count("outputPaths"))
      cbs.savePaths(vm["outputPaths"].as<string>());
    if (vm["stats"].as<bool>()) cbs.saveStats(vm["output"].as<string>(), vm["agents"].as<string>());

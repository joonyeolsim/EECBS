...
bool is_edge_mutex(edge_pair ep) { return ep.first.second != nullptr; }
...
void ConstraintPropagation::add_fwd_edge_mutex(MDDNode* node_a, MDDNode* node_a_to, MDDNode* node_b,
                                               MDDNode* node_b_to) {
...
void ConstraintPropagation::fwd_mutex_prop() {
  ...
  while (!open.empty()) {
    ...
    if (is_edge_mutex(mutex)) {
      ...
      auto new_mutex =
          std::make_pair(std::make_pair(node_to_1, nullptr), std::make_pair(node_to_2, nullptr));
...
ConstraintPropagation::generate_constraints(int level_0, int level_1) {
  ...
  if (!non_mutexed.empty()) {
    ...
    cons_vec_1.push_back(
        Constraint(0, goal_ptr_i->location, -1, level_0 - 1, constraint_type::LEQLENGTH));

...
Path SIPP::findPath(const ConstraintTable& constraint_table) {
  ...
  while (!focal_list.empty()) {
         ...
         instance.getNeighbors(curr->location))  // move to neighboring locations
    {
      for (auto& i : reservation_table.get_safe_intervals(
        ...
        tie(next_high_generation, next_timestep, next_high_expansion, next_v_collision,
            next_e_collision) = i;
        if (next_timestep + my_heuristic[next_location] > constraint_table.length_max) break;
    ...
    if (curr->high_expansion == curr->high_generation and
      ...
      auto next_h_val = max(curr->h_val, (get<2>(interval) ? holding_time : curr->getFVal()) -
                                             next_timestep);  // path max
      auto next_collisions = curr->num_of_conflicts +
                             (int)curr->collision_v * max(next_timestep - curr->timestep - 1, 0) +
                             (int)get<2>(interval);
      auto next =
          new SIPPNode(curr->location, next_timestep, next_h_val, curr, next_timestep,
...
// minimizing the number of internal conflicts (that is conflicts with known_paths for other agents
// found so far). lowerbound is an underestimation of the length of the path in order to speed up
// the search.
pair<Path, int> SIPP::findSuboptimalPath(const HLNode& node,
  ...
  num_generated = 0;
  Interval interval = reservation_table.get_first_safe_interval(start_location);
  if (get<0>(interval) > 0) return {path, 0};

  // generate start and add it to the OPEN list
  auto start =
      new SIPPNode(start_location, 0, max(my_heuristic[start_location], holding_time), nullptr, 0,
  ...
  while (!open_list.empty()) {
         ...
         instance.getNeighbors(curr->location))  // move to neighboring locations
    {
      for (auto& i : reservation_table.get_safe_intervals(
        ...
        int next_h_val =
            max(my_heuristic[next_location], curr->getFVal() - next_g_val);  // path max
        if (next_g_val + next_h_val > reservation_table.constraint_table.length_max) continue;
    ...
    if (curr->high_expansion == curr->high_generation and
      ...
      auto next =
          new SIPPNode(curr->location, next_timestep, next_h_val, curr, next_timestep,
                       get<1>(interval), get<1>(interval), get<2>(interval), next_collisions);
      if (curr->location == goal_location) next->wait_at_goal = true;
      if (dominanceCheck(next))
        pushNodeToOpenAndFocal(next);
      else
        delete next;
...
/*Path SIPP::findNoCollisionPath(const ConstraintTable& constraint_table)
{
    ...
    if (get<0>(interval) > 0 or get<2>(interval))
        return path;
    auto holding_time = max(constraint_table.getHoldingTime(goal_location,
constraint_table.length_min), constraint_table.getLastCollisionTimestep(goal_location) + 1);
    // generate start and add it to the OPEN & FOCAL list
    ...
    while (!focal_list.empty())
    {
        ...
        if (curr->location == goal_location && // arrive at the goal location
            !curr->wait_at_goal && // not wait at the goal location
            curr->timestep >= holding_time && // the agent can hold the goal location afterward
            constraint_table.getFutureNumOfCollisions(curr->location, curr->timestep) == 0) // no
future collisions
        {
            updatePath(curr, path);
            break;
        }
        for (int next_location : instance.getNeighbors(curr->location)) // move to neighboring
locations
        {
                    ...
                    curr->location, next_location, curr->timestep + 1, get<1>(curr->interval) + 1))
            {
                ...
                if (interval.second + next_h_val > constraint_table.length_max)
                    break;
                generateChildToFocal(interval.first, curr, next_location, interval.second,
next_h_val);
            }
        }  // end for loop that generates successors
        // wait at the current location
        bool found = reservation_table.find_safe_interval(interval, curr->location,
get<1>(curr->interval)); if (found and !get<2>(interval))
        {
...
int SIPP::getTravelTime(int start, int end, const ConstraintTable& constraint_table,
  ...
  >::handle_type open_handle; unordered_set<SIPPNode*, SIPPNode::NodeHasher, SIPPNode::eqnode>
  nodes;
  ...
  interval); root->open_handle = open_list.push(root);  // add root to heap nodes.insert(root); //
  ...
  while (!open_list.empty())
  {
      ...
      for (int next_location : instance.getNeighbors(curr->location))
      {
          if ((curr->location == blocked.first && next_location == blocked.second) ||
              (curr->location == blocked.second && next_location == blocked.first)) // use the
  prohibited edge
          {
              continue;
          }
              ...
              curr->location, next_location, curr->timestep + 1, get<1>(curr->interval) + 1))
          {
              ...
              if (next_g_val + next_h_val >= upper_bound) // the cost of the path is larger than the
  upper bound continue; auto next = new SIPPNode(next_location, next_g_val, next_h_val, nullptr,
  next_timestep, interval); auto it = nodes.find(next); if (it == nodes.end()) {  // add the newly
  generated node to heap and hash table next->open_handle = open_list.push(next);
                  nodes.insert(next);
              }
  ...
  for (auto& old_node : ptr->second) {
    if (old_node->timestep <= new_node->timestep and
        old_node->num_of_conflicts <=
            new_node->num_of_conflicts) {  // the new node is dominated by the old node
      return false;
    ...
    {                                           // delete the old node
      ...
      num_generated--;  // this is because we later will increase num_generated when we insert the
                        // new node into lists.
      return true;
    } else if (old_node->timestep < new_node->high_expansion and
               new_node->timestep <
                   old_node->high_expansion) {  // intervals overlap --> we need to split the node

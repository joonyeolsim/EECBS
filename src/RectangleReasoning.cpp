...
shared_ptr<Conflict> RectangleReasoning::run(const vector<Path*>& paths, int timestep, int a1,
                                             int a2, const MDD* mdd1, const MDD* mdd2) {
  clock_t t = clock();
...
shared_ptr<Conflict> RectangleReasoning::findRectangleConflictByGR(const vector<Path*>& paths,
  ...
  if (!haveBarriers) return nullptr;
  ...
  haveBarriers =
      ExtractBarriers(*mdd2, loc, timestep, loc - from2, loc - from1,
  ...
  int Rg_t = timestep + Utils::getManhattanDistance(Utils::getCoordinate(loc, env), Rg);
  ...
  {
    succ = addModifiedVerticalBarrierConstraint(a1, mdd1, Rg.second, Rs.first, Rg.first, Rg_t,
                                                constraint1);
    assert(succ);
    succ = addModifiedHorizontalBarrierConstraint(a2, mdd2, Rg.first, Rs.second, Rg.second, Rg_t,
                                                  constraint2);
    assert(succ);
  } else {
    succ = addModifiedHorizontalBarrierConstraint(a1, mdd1, Rg.first, Rs.second, Rg.second, Rg_t,
                                                  constraint1);
    assert(succ);
    succ = addModifiedVerticalBarrierConstraint(a2, mdd2, Rg.second, Rs.first, Rg.first, Rg_t,
                                                constraint2);
    assert(succ);
  }

  if (!blocked(*paths[a1], constraint1) || !blocked(*paths[a2], constraint2)) return nullptr;
...
shared_ptr<Conflict> RectangleReasoning::findRectangleConflictByRM(const vector<Path*>& paths,
                                                                   int timestep, int a1, int a2,
                                                                   const MDD* mdd1,
                                                                   const MDD* mdd2) {
  ...
  pair<int, int> location = Utils::getCoordinate(paths[a1]->at(timestep).location, env);
  ...
  for (int t1_start : s1s) {
    for (int t1_end : g1s) {
      auto s1 = Utils::getCoordinate(paths[a1]->at(t1_start).location, env);
      auto g1 = Utils::getCoordinate(paths[a1]->at(t1_end).location, env);
      if (Utils::getManhattanDistance(s1, g1) != t1_end - t1_start) continue;
      for (int t2_start : s2s) {
        for (int t2_end : g2s) {
          auto s2 = Utils::getCoordinate(paths[a2]->at(t2_start).location, env);
          auto g2 = Utils::getCoordinate(paths[a2]->at(t2_end).location, env);
          if (Utils::getManhattanDistance(s2, g2) != t2_end - t2_start) continue;
          if (!isRectangleConflict(s1, s2, g1, g2)) continue;
          ...
          if (new_type > type || (new_type == type && new_area > area)) {
            ...
            list<Constraint> constraint1;
            list<Constraint> constraint2;
            bool succ = addModifiedBarrierConstraints(a1, a2, Rs, Rg, s1, s2, Rg_t, mdd1, mdd2,
...
bool RectangleReasoning::ExtractBarriers(const MDD& mdd, int loc, int timestep, int dir1, int dir2,
                                         int start, int goal, int start_time, list<Constraint>& B) {
  int num_barrier;
  int sign1 = dir1 / abs(dir1);
  int sign2 = dir2 / abs(dir2);
  if (abs(dir1) == 1)  // vertical barriers
  {
    num_barrier = sign1 * (Utils::getColCoordinate(goal, env) - Utils::getColCoordinate(start, env)) + 1;
  } else {
    num_barrier = sign1 * (Utils::getRowCoordinate(goal, env) - Utils::getRowCoordinate(start, env)) + 1;
  }
  ...
  if (abs(dir1) == 1)  // vertical barriers
  {
    barrier_time =
        timestep +
        sign1 * (Utils::getColCoordinate(n->location, env) - Utils::getColCoordinate(loc, env)) +
        sign2 * (Utils::getRowCoordinate(n->location, env) - Utils::getRowCoordinate(loc, env));
  } else {
    barrier_time =
        timestep +
        sign1 * (Utils::getRowCoordinate(n->location, env) - Utils::getRowCoordinate(loc, env)) +
        sign2 * (Utils::getColCoordinate(n->location, env) - Utils::getColCoordinate(loc, env));
  }
  if (barrier_time == 0) {
    extent_L[0] = 0;
    extent_U[0] = 0;
    ...
    for (auto n : mdd.levels[t]) {
      ...
      for (auto parent : n->parents) {
        ...
        for (int i = 0; i < num_barrier; i++) {
          if (!parent_block[i]) block[i] = false;
        }
      }
      ...
      {
        barrier_id =
            sign1 * (Utils::getColCoordinate(n->location, env) - Utils::getColCoordinate(start, env));
        barrier_time =
            timestep +
            sign1 * (Utils::getColCoordinate(n->location, env) - Utils::getColCoordinate(loc, env)) +
            sign2 * (Utils::getRowCoordinate(n->location, env) - Utils::getRowCoordinate(loc, env));
        ...
        barrier_id =
            sign1 * (Utils::getRowCoordinate(n->location, env) - Utils::getRowCoordinate(start, env));
        barrier_time =
            timestep +
            sign1 * (Utils::getRowCoordinate(n->location, env) - Utils::getRowCoordinate(loc, env)) +
            sign2 * (Utils::getColCoordinate(n->location, env) - Utils::getColCoordinate(loc, env));
  ...
  for (int i = 0; i < num_barrier; i++) {
    if (block[i]) {
      ...
      if (abs(dir1) == 1)  // vertical barriers
      {
        barrier_start_y = Utils::getColCoordinate(start, env) + sign1 * i;
        barrier_end_y = barrier_start_y;
        int time_offset =
            timestep + i -
            sign1 * (Utils::getColCoordinate(loc, env) - Utils::getColCoordinate(start, env));
        barrier_start_x = Utils::getRowCoordinate(loc, env) + sign2 * (extent_L[i] - time_offset);
        barrier_end_x = Utils::getRowCoordinate(loc, env) + sign2 * (extent_U[i] - time_offset);
      } else {
        barrier_start_x = Utils::getRowCoordinate(start, env) + sign1 * i;
        barrier_end_x = barrier_start_x;
        int time_offset =
            timestep + i -
            sign1 * (Utils::getRowCoordinate(loc, env) - Utils::getRowCoordinate(start, env));
        barrier_start_y = Utils::getColCoordinate(loc, env) + sign2 * (extent_L[i] - time_offset);
        barrier_end_y = Utils::getColCoordinate(loc, env) + sign2 * (extent_U[i] - time_offset);
      }
      barrier_end_time = extent_U[i];
      B.emplace_back(
          -1,  // for now, the agent index is not important,  so we just use -1 for simplicity.
          Utils::linearizeCoordinate(barrier_start_x, barrier_start_y, env),
          Utils::linearizeCoordinate(barrier_end_x, barrier_end_y, env), barrier_end_time,
...
bool RectangleReasoning::isEntryBarrier(const Constraint& b1, const Constraint& b2, int dir1) {
  pair<int, int> b1_l = Utils::getCoordinate(get<1>(b1), env);
  pair<int, int> b1_u = Utils::getCoordinate(get<2>(b1), env);
...
bool RectangleReasoning::blockedNodes(const Path& path, const pair<int, int>& Rs,
  ...
  for (int t = t_min; t <= t_max; t++) {
    int loc = Utils::linearizeCoordinate(b_l.first, b_l.second, env) + (t - t_b_l) * dir;
...
bool RectangleReasoning::isCut(const Constraint& b, const pair<int, int>& Rs,
                               const pair<int, int>& Rg) {
  pair<int, int> b_l = Utils::getCoordinate(get<1>(b), env);
  pair<int, int> b_u = Utils::getCoordinate(get<2>(b), env);
  if (b_l == b_u) {
  ...
  if (Rs.first <= b_l.first && b_l.first <= b_u.first && b_u.first <= Rg.first &&
      b_l.second == b_u.second)
    return true;
  else if (Rs.second <= b_l.second && b_l.second <= b_u.second && b_u.second <= Rg.second &&
           b_l.first == b_u.first)
    return true;
  else if (Rs.first >= b_l.first && b_l.first >= b_u.first && b_u.first >= Rg.first &&
           b_l.second == b_u.second)
    return true;
  else if (Rs.second >= b_l.second && b_l.second >= b_u.second && b_u.second >= Rg.second &&
           b_l.first == b_u.first)
...
void RectangleReasoning::generalizedRectangle(const Path& path1, const Path& path2, const MDD& mdd1,
    ...
    for (const auto& b2_entry : B2) {
      if (isEntryBarrier(b1_entry, b2_entry, dir1) && isEntryBarrier(b2_entry, b1_entry, dir2)) {
        ...
        while (b1_exit != B1.rend() && b2_exit != B2.rend()) {
          ...
          int Rg_t = timestep + Utils::getManhattanDistance(Rg, Utils::getCoordinate(loc, env));
...
bool RectangleReasoning::isRectangleConflict(const pair<int, int>& s1, const pair<int, int>& s2,
  ...
  if ((s1.first - g1.first) * (s2.first - g2.first) < 0 ||
    ...
    return false;
  /*else if ((S2.first - S1.first) * (S1.first - G1.first) > 0 && (S2.second - S1.second) *
  (S1.second - G1.second) > 0) // s1 always in the middle return false; else if ((S1.first -
  S2.first) * (S2.first - G2.first) > 0 && (S1.second - S2.second) * (S2.second - G2.second) > 0) //
  s2 always in the middle return false;*/
  else
...
int RectangleReasoning::classifyRectangleConflict(const pair<int, int>& s1,
                                                  const pair<int, int>& s2,
                                                  const pair<int, int>& g1,
                                                  const pair<int, int>& g2) {
  int cardinal1 = 0, cardinal2 = 0;
  if ((s1.first - s2.first) * (g1.first - g2.first) <= 0) cardinal1++;
  if ((s1.second - s2.second) * (g1.second - g2.second) <= 0) cardinal2++;
...
int RectangleReasoning::classifyRectangleConflict(const pair<int, int>& s1,
  ...
  if ((s1.first == s2.first && (s1.second - s2.second) * (s2.second - Rg.second) >= 0) ||
    ...
    if (Rg.first == g1.first) cardinal1 = 1;
    if (Rg.second == g2.second) cardinal2 = 1;
  } else {
    if (Rg.second == g1.second) cardinal1 = 1;
    if (Rg.first == g2.first) cardinal2 = 1;
  }
...
pair<int, int> RectangleReasoning::getRs(const pair<int, int>& s1, const pair<int, int>& s2,
                                         const pair<int, int>& g1) {
  int x, y;
...
pair<int, int> RectangleReasoning::getRg(const pair<int, int>& s1, const pair<int, int>& g1,
                                         const pair<int, int>& g2) {
  int x, y;
  if (s1.first == g1.first)
    x = g1.first;
...
list<int> RectangleReasoning::getStartCandidates(const Path& path, const MDD& mdd, int timestep) {
  ...
  for (int t = 0; t <= timestep;
       t++)  // Find start that is single and Manhattan-optimal to conflicting location
  {
    if (mdd.levels[t].size() == 1 && mdd.levels[t].front()->location == path[t].location &&
        Utils::getManhattanDistance(path[t].location, path[timestep].location, env) == timestep - t)
      starts.push_back(t);
  }
  return starts;
}
...
list<int> RectangleReasoning::getGoalCandidates(const Path& path, const MDD& mdd, int timestep) {
  ...
  for (int t = (int)path.size() - 1; t >= timestep;
       t--)  // Find end that is single and Manhattan-optimal to conflicting location
  {
    if (mdd.levels[t].size() == 1 && mdd.levels[t].front()->location == path[t].location &&
        Utils::getManhattanDistance(path[t].location, path[timestep].location, env) == t - timestep)
      goals.push_back(t);
  }
  return goals;
}
...
int RectangleReasoning::getStartCandidate(const Path& path, int dir1, int dir2, int timestep) {
  for (int t = timestep; t > 0;
       t--)  // Find the earliest start that is single and Manhattan-optimal to conflicting location
  {
...
int RectangleReasoning::getGoalCandidate(const Path& path, int dir1, int dir2, int timestep) {
  for (int t = timestep; t < (int)path.size() - 1;
       t++)  // Find the latest end that is single and Manhattan-optimal to conflicting location
  {
...
/*int getRectangleTime(const Conflict& conflict, const std::vector<std::vector<PathEntry>*>& paths,
int num_col)
{
        ...
        std::pair<int, int> Rg = std::make_pair(std::get<2>(conflict) / num_col,
std::get<2>(conflict) % num_col); std::pair<int, int> Rs = getRs(S1, S2, Rg); return
std::get<3>(conflict) - abs(S1.first - Rs.first) - abs(S1.second - Rs.second);
}*/

bool RectangleReasoning::addModifiedBarrierConstraints(
    int a1, int a2, const pair<int, int>& Rs, const pair<int, int>& Rg, const pair<int, int>& s1,
    const pair<int, int>& s2, int Rg_t, const MDD* mdd1, const MDD* mdd2,
  ...
  {
    int Rs_t = Rg_t - Utils::getManhattanDistance(Rs, Rg);
    ...
    if (!found) {
      ...
      if (!addModifiedHorizontalBarrierConstraint(a1, mdd1, Rg.first, Rs.second, Rg.second, Rg_t,
                                                  constraint1))
        return false;
      if (!addModifiedVerticalBarrierConstraint(a2, mdd2, Rg.second, Rs.first, Rg.first, Rg_t,
                                                constraint2))
        return false;
      return true;
    }
    ...
    if (!found) {
      ...
      if (!addModifiedVerticalBarrierConstraint(a1, mdd1, Rg.second, Rs.first, Rg.first, Rg_t,
                                                constraint1))
        return false;
...
bool RectangleReasoning::hasNodeOnBarrier(const MDD* mdd, int y_start, int y_end, int x, int t_min,
  ...
  for (int t2 = t_min + 1; t2 <= min(t_max, (int)mdd->levels.size() - 1); t2++) {
    if (horizontal)
      loc = Utils::linearizeCoordinate(x, y_start + (t2 - t_min) * sign, env);
    else
      loc = Utils::linearizeCoordinate(y_start + (t2 - t_min) * sign, x, env);
...
bool RectangleReasoning::addModifiedHorizontalBarrierConstraint(int agent, const MDD* mdd, int x,
  ...
  for (int t2 = t_min; t2 <= t_max; t2++) {
    int loc = Utils::linearizeCoordinate(x, (Ri_y + (t2 - Ri_t) * sign), env);
    MDDNode* it = nullptr;
    for (MDDNode* n : mdd->levels[t2]) {
      if (n->location == loc) {
        it = n;
    ...
    {
      int loc1 = Utils::linearizeCoordinate(x, (Ri_y + (t1 - Ri_t) * sign), env);
      int loc2 = Utils::linearizeCoordinate(x, (Ri_y + (t2 - 1 - Ri_t) * sign), env);
    ...
    if (it != nullptr && t2 == t_max) {
      int loc1 = Utils::linearizeCoordinate(x, (Ri_y + (t1 - Ri_t) * sign), env);
      constraints.emplace_back(agent, loc1, loc, t2,
...
bool RectangleReasoning::addModifiedVerticalBarrierConstraint(int agent, const MDD* mdd, int y,
  ...
  int t_max = min(Rg_t, (int)mdd->levels.size() - 1);
  for (int t2 = t_min; t2 <= t_max; t2++) {
    int loc = Utils::linearizeCoordinate((Ri_x + (t2 - Ri_t) * sign), y, env);
    MDDNode* it = nullptr;
    for (MDDNode* n : mdd->levels[t2]) {
      if (n->location == loc) {
        it = n;
    ...
    if (it == nullptr && t1 >= 0)  // add constraints [t1, t2)
    {
      int loc1 = Utils::linearizeCoordinate((Ri_x + (t1 - Ri_t) * sign), y, env);
      int loc2 = Utils::linearizeCoordinate((Ri_x + (t2 - 1 - Ri_t) * sign), y, env);
      constraints.emplace_back(agent, loc1, loc2, t2 - 1, constraint_type::BARRIER);
      t1 = -1;
      continue;
    } else if (it != nullptr && t1 < 0) {
...
bool RectangleReasoning::blocked(const Path& path, const list<Constraint>& constraints) {
  for (auto constraint : constraints) {
    ...
    int x1 = Utils::getRowCoordinate(x, env), y1 = Utils::getColCoordinate(x, env);
    int x2 = Utils::getRowCoordinate(y, env), y2 = Utils::getColCoordinate(y, env);
    if (x1 == x2) {
      if (y1 < y2) {
        for (int i = 0; i <= min(y2 - y1, t); i++) {
          if (traverse(path, Utils::linearizeCoordinate(x1, y2 - i, env), t - i)) return true;
        }
      } else {
    ...
    } else  // y1== y2
    {
      if (x1 < x2) {
        for (int i = 0; i <= min(x2 - x1, t); i++) {
          if (traverse(path, Utils::linearizeCoordinate(x2 - i, y1, env), t - i)) return true;
        }
      } else {

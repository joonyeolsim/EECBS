...
bool ECBS::solve(double time_limit, int _cost_lowerbound) {
  ...
  while (!cleanup_list.empty() && !solution_found) {
        ...
        !curr->h_computed)  // heuristics has not been computed yet
    {
      ...
      bool succ =
          heuristic_helper.computeInformedHeuristics(*curr, min_f_vals, time_limit - runtime);
      runtime = (double)(clock() - start) / CLOCKS_PER_SEC;
      ...
      while (foundBypass) {
        ...
        for (int i = 0; i < 2; i++) {
          ...
          if (!solved[i]) {
            ...
            continue;
          else if (bypass && child[i]->sum_of_costs <= suboptimality * cost_lowerbound &&
...
// start also, do the same for ll_min_f_vals and paths_costs (since its already "on the way").
...
bool ECBS::generateRoot() {
  ...
  for (auto i : agents) {
    paths_found_initially[i] = search_engines[i]->findSuboptimalPath(*root, initial_constraints[i],
                                                                     paths, i, 0, suboptimality);
...
bool ECBS::generateChild(ECBSNode* node, ECBSNode* parent) {
  ...
  for (auto agent : agents) {
    if (!findPathForSingleAgent(node, agent)) {
      if (screen > 1) cout << "	No paths for agent " << agent << ". Node pruned." << endl;
...
inline bool ECBS::reinsertNode(ECBSNode* node) {
  switch (solver_type) {
    case high_level_solver_type::ASTAREPS:
      if (node->sum_of_costs <= suboptimality * cost_lowerbound) return false;
      node->cleanup_handle = cleanup_list.push(node);
      break;
    case high_level_solver_type::NEW:
      if (node->getFHatVal() <= suboptimality * cost_lowerbound) return false;
      node->cleanup_handle = cleanup_list.push(node);
      break;
    case high_level_solver_type::EES:
      node->cleanup_handle = cleanup_list.push(node);
    ...
    case high_level_solver_type::EES:
      ...
      if (open_list.top()->getFHatVal() != inadmissible_cost_lowerbound) {
        ...
        for (auto n : open_list) {
          if (n->getFHatVal() <= focal_list_threshold) n->focal_handle = focal_list.push(n);
        }
      }

      // choose the best node
      if (screen > 1 && cleanup_list.top()->getFVal() > cost_lowerbound)
        cout << "Lowerbound increases from " << cost_lowerbound << " to "
             << cleanup_list.top()->getFVal() << endl;
    ...
    case high_level_solver_type::NEW:
      ...
      if (cleanup_list.top()->getFVal() > cost_lowerbound) {
        ...
        for (auto n : cleanup_list) {
          ...
          if (n->getFHatVal() <= new_focal_list_threshold) n->focal_handle = focal_list.push(n);
        }
        if (screen == 3) {
          cout << focal_list.size() << endl;
        }
...
void ECBS::classifyConflicts(ECBSNode& node) {
  ...
  while (!node.unknownConf.empty()) {
    ...
    /*if (con->priority == conflict_priority::CARDINAL && heuristic_helper.type ==
    heuristics_type::ZERO)
    {
    ...
    if (rectangle_reasoning &&
        (int)paths[a1]->size() - 1 ==
            min_f_vals[a1] &&  // the paths for both agents are their shortest paths

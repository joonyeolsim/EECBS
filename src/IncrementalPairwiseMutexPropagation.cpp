...
using namespace std;
IPMutexPropagation::IPMutexPropagation(MDD* MDD_0, MDD* MDD_1, SingleAgentSolver* se_0,
                                       SingleAgentSolver* se_1, ConstraintTable cons_0,
                                       ConstraintTable cons_1, int incr_limit)
    : MDD_0(MDD_0),
      MDD_1(MDD_1),
      init_len_0(MDD_0->levels.size()),
      init_len_1(MDD_1->levels.size()),
      incr_limit(incr_limit),
...
std::pair<con_vec, con_vec> IPMutexPropagation::gen_constraints() {
  ...
  while (!found_solution) {
    ...
    if (inc_len > incr_limit) {
      ...
      return cp.generate_constraints(init_len_0 + max(inc_len - 1, 0),
                                     init_len_1 + max(inc_len - 1, 0));
    }
    ...
    if (!cp.feasible(init_len_0 + inc_len - 1, init_len_1 + inc_len - 1)) {
      ...
      if (init_len_0 < init_len_1) {
        ...
        while (init_len_0 + inc_0 <= init_len_1 + inc_1 &&
          ...
          inc_0 += 1;
          MDD_0->increaseBy(cons_0, 1, search_engine_0);
          // cout <<  init_len_0 + inc_0 +  max(inc_len - 1, 0) - 1 << " " << init_len_1 + inc_1 +
          // max(inc_len - 1, 0) - 1 << endl; cout << cp.feasible( init_len_0 + inc_0 +  max(inc_len
          // - 1, 0) - 1, init_len_1 + inc_1 + max(inc_len - 1, 0) - 1) << endl;
        }
        // cout << "inreasing a0 " << inc_0 << endl;
        // cout << init_len_0 + max(inc_0 - 1, 0) + max(inc_len - 1, 0) - 1 << " " << init_len_1 +
        // max(inc_len - 1, 0) - 1 << endl;

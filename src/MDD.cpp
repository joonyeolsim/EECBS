...
/*bool MDD::isConstrained(int curr_id, int next_id, int next_timestep, const std::vector< std::list<
std::pair<int, int> > >& cons)  const
{
        if (cons.empty())
                return false;
        ...
        if (next_timestep < static_cast<int>(cons.size()))
        {
                for (const auto & it : cons[next_timestep])
                {
                        if ((std::get<0>(it) == next_id && std::get<1>(it) < 0)//vertex constraint
                                || (std::get<0>(it) == curr_id && std::get<1>(it) == next_id)) //
edge constraint return true;
                }
        }
        return false;
}*/
bool MDD::buildMDD(ConstraintTable& constraint_table, const SingleAgentSolver* _solver) {
  struct Node {
    ...
    // Node() = default;
    Node(int location, int timestep, int h_val)
        : location(location), timestep(timestep), h_val(h_val) {}
  };
  this->solver = _solver;
  int holding_time = constraint_table.getHoldingTime(
      solver->goal_location,
      constraint_table.length_min);  // the earliest timestep that the agent can hold its goal
                                     // location. The length_min is considered here.
  auto root =
      new Node(solver->start_location, 0, solver->my_heuristic[solver->start_location]);  // Root
  ...
  while (!open.empty()) {
        ...
        curr->timestep >= holding_time)  // the agent can hold the goal location afterward
    {
      if (curr->parents.size() != 1 ||
          curr->parents.front()->location !=
              solver->goal_location) {  // skip the case where curr only have parent node who
                                        // locates at goal_location
        goal_node = curr;
        upperbound = curr->timestep;
        continue;
      }
    }
    if (curr->timestep + curr->h_val > upperbound) continue;
    auto next_locations = solver->getNextLocations(curr->location);
    for (int next_location :
         next_locations)  // Try every possible move. We only add backward edges in this step.
    {
      ...
      if (next_timestep + next_h_val > upperbound) continue;
      ...
      } else  // If the child node exists
      {
        ...
        if (goal_node == nullptr &&
            ...
            curr->location != solver->goal_location)  // skip the case where curr only have parent
                                                      // node who locates at goal_location
        {
          goal_node = (*it);
  ...
  // release memory
  for (auto it : allNodes_table) delete it;
...
bool MDD::buildMDD(const ConstraintTable& ct, int num_of_levels, const SingleAgentSolver* _solver) {
  ...
  while (!open.empty()) {
    ...
    // We want (g + 1)+h <= f = numOfLevels - 1, so h <= numOfLevels - g - 2. -1 because it's the
    // bound of the children.
  ...
  for (auto it : closed)
    if (it->children.empty() && it->level < num_of_levels - 1) delete it;
  closed.clear();
  assert(levels.back().front()->location == solver->goal_location);
  return true;
}

/*bool MDD::buildMDD(const std::vector <std::list< std::pair<int, int> > >& constraints, int
numOfLevels, int start_location, const int* moves_offset, const std::vector<int>& my_heuristic, int
map_size, int num_col)
{
        ...
        while (!open.empty())
        {
                ...
                // We want (g + 1)+h <= f = numOfLevels - 1, so h <= numOfLevels - g. -1 because
it's the bound of the children. double heuristicBound = numOfLevels - node->level - 2 + 0.001; for
(int i = 0; i < 5; i++) // Try every possible move. We only add backward edges in this step.
                {
                        ...
                        if (validMove(node->location, newLoc, map_size, num_col) &&
                                ...
                                !isConstrained(node->location, newLoc, node->level + 1,
constraints)) // valid move
                        {
                                auto child = closed.rbegin();
                                bool find = false;
                                for (; child != closed.rend() && ((*child)->level == node->level +
1); ++child) if ((*child)->location == newLoc) // If the child node exists
                                        {
                                                (*child)->parents.push_back(node); // then add
corresponding parent link and child link find = true; break;
                                        }
                                if (!find) // Else generate a new mdd node
                                {
        ...
        for (int t = numOfLevels - 1; t > 0; t--)
        {
                ...
                {
                        for (auto parent = (*it)->parents.begin(); parent != (*it)->parents.end();
parent++)
                        {
                                if ((*parent)->children.empty()) // a new node
                                {
...
void MDD::deleteNode(MDDNode* node) {
  ...
  for (auto child = node->children.begin(); child != node->children.end(); ++child) {
    ...
    if ((*child)->parents.empty()) deleteNode(*child);
  }
  for (auto parent = node->parents.begin(); parent != node->parents.end(); ++parent) {
    (*parent)->children.remove(node);
    if ((*parent)->children.empty()) deleteNode(*parent);
  }
}
...
void MDD::increaseBy(const ConstraintTable& ct, int dLevel, SingleAgentSolver* solver) {
  ...
  for (int l = 0; l < numOfLevels - 1; l++) {
    ...
    for (auto& it : levels[l]) {
      ...
      // for (int i = 0; i < 5; i++) // Try every possible move. We only add backward edges in this
      // step.
      {
...
void SyncMDD::deleteNode(SyncMDDNode* node, int level) {
  ...
  for (auto child = node->children.begin(); child != node->children.end(); ++child) {
    (*child)->parents.remove(node);
    if ((*child)->parents.empty()) deleteNode(*child, level + 1);
  }
  for (auto parent = node->parents.begin(); parent != node->parents.end(); ++parent) {
    (*parent)->children.remove(node);
    if ((*parent)->children.empty()) deleteNode(*parent, level - 1);
  }
}
...
void MDDTable::releaseMDDMemory(int id) {
  if (id < 0 || lookupTable.empty() || (int)lookupTable[id].size() < max_num_of_mdds) return;
  int minLength = MAX_TIMESTEP;
  for (auto mdd : lookupTable[id]) {
    if ((int)mdd.second->levels.size() < minLength) minLength = mdd.second->levels.size();
  }

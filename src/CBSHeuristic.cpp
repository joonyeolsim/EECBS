// #pragma warning(disable: 4996) //Jiaoyang: I added this line to disable error C4996 caused by
// CPLEX
...
void CBSHeuristic::updateInadmissibleHeuristics(HLNode& curr) {
  ...
  switch (inadmissible_heuristic) {
    ...
    case heuristics_type::GLOBAL:  // Note: there is no "break" in the previous line, so here we
                                   // compute heuristics for both GLOBAL and PATH
    ...
    case heuristics_type::LOCAL:
      if (std::abs(1 - sum_distance_errors[0]) < 0.001)
        ...
        curr.cost_to_go =
            h +
            max(0, (int)(curr.distance_to_go * sum_cost_errors[0] / (1 - sum_distance_errors[0])));
    ...
    case heuristics_type::CONFLICT:
      if (curr.conflicts.empty() && curr.unknownConf.empty()) return;
      ...
      {
              auto agents = make_pair(min(conflict->a1, conflict->a2), max(conflict->a1,
      conflict->a2)); if (conflicting_agents.find(agents) != conflicting_agents.end()) continue; //
      we do not recount the conflict between the same pair of agents int id =
      conflict->getConflictId(); cost_error += getCostError(id); distance_error +=
      getDistanceError(id);
      }*/
...
void CBSHeuristic::updateOnlineHeuristicErrors(CBSNode& curr) {
  if ((inadmissible_heuristic == heuristics_type::GLOBAL ||
       ...
       inadmissible_heuristic == heuristics_type::CONFLICT) &&
      curr.parent != nullptr) {
    ...
    if (curr.parent->fully_expanded)  // update error
    {
      ...
      } else if (inadmissible_heuristic == heuristics_type::LOCAL)  // update local error
      {
        ...
        sum_distance_errors[0] = sum_distance_errors[0] * (1 - learning_rate) +
                                 curr.parent->distance_error * learning_rate;
        sum_cost_errors[0] =
...
void CBSHeuristic::updateOnlineHeuristicErrors(ECBSNode& parent) {
  if (inadmissible_heuristic == heuristics_type::ZERO) return;
  // Find the best child
  ...
  if (parent.children.size() == 2) {
    ...
    if (best_child->getFHatVal() > other->getFHatVal() ||
        (best_child->getFHatVal() == other->getFHatVal() &&
         best_child->distance_to_go > other->distance_to_go))
      best_child = other;
  }
  // Update the errors
  ...
  } else if (inadmissible_heuristic == heuristics_type::LOCAL)  // update local error
  {
    ...
    sum_cost_errors[0] =
        sum_cost_errors[0] * (1 - learning_rate) + parent.cost_error * learning_rate;
  } else if (inadmissible_heuristic == heuristics_type::CONFLICT)  // update conflict error
  {
...
void CBSHeuristic::computeQuickHeuristics(HLNode& node) {
  ...
  {
  if ((e.first / num_of_agents == agent || e.first % num_of_agents == agent) && e.second >
  maxWeight)
  {
  maxWeight = e.second;
  ...
  switch (type) {
    ...
    case heuristics_type::CG:
      ...
      if (curr.parent == nullptr ||
          num_of_CGedges > ILP_edge_threshold ||  // root node of CBS tree or the graph is too large
          target_reasoning ||
          disjoint_splitting)  // when we are allowed to replan for multiple agents, the incremental
                               // method is not correct any longer.
        h = minimumVertexCover(HG);
      else
        h = minimumVertexCover(HG, curr.parent->h_val, num_of_agents, num_of_CGedges);
      break;
    case heuristics_type::DG:
      if (!buildDependenceGraph(curr, HG, num_of_CGedges)) return false;
      // Minimum Vertex Cover
      if (curr.parent == nullptr ||
          num_of_CGedges > ILP_edge_threshold ||  // root node of CBS tree or the graph is too large
          target_reasoning ||
...
bool CBSHeuristic::computeInformedHeuristics(ECBSNode& curr, const vector<int>& min_f_vals,
                                             double _time_limit) {
  curr.h_computed = true;
  // create conflict graph
  start_time = clock();
  ...
  switch (type) {
    ...
    /*case heuristics_type::CG:
            buildCardinalConflictGraph(curr, HG, num_of_CGedges);
            // Minimum Vertex Cover
            if (curr.parent == nullptr || num_of_CGedges > ILP_edge_threshold) // root node of CBS
    tree or the graph is too large h = minimumVertexCover(HG); else h = minimumVertexCover(HG,
    curr.parent->h_val, num_of_agents, num_of_CGedges); break; case heuristics_type::DG: if
    (!buildDependenceGraph(curr, HG, num_of_CGedges)) return false;
            // Minimum Vertex Cover
            if (curr.parent == nullptr || num_of_CGedges > ILP_edge_threshold) // root node of CBS
    tree or the graph is too large h = minimumVertexCover(HG); else h = minimumVertexCover(HG,
    curr.parent->h_val, num_of_agents, num_of_CGedges); break;*/
    case heuristics_type::WDG:
      int delta_g;
      if (!buildWeightedDependencyGraph(curr, min_f_vals, HG, delta_g)) return false;
      assert(delta_g >= 0);
  ...
  curr.h_val = max(h, curr.h_val);
  curr.cost_to_go =
      max(curr.cost_to_go, curr.getFVal() - curr.sum_of_costs);  // ensure that f <= f^
  // if (screen == 2)
  //	curr.printConflictGraph(num_of_agents);
        ...
        for (int i = 0; i < num_of_agents; i++)
        {
                ...
                while (!Q.empty())
                {
                        ...
                        for (int k = 0; k < num_of_agents; k++)
                        {
                                ...
                                {
                                        range[num] = std::max(range[num], HG[j * num_of_agents +
k]); if (!done[k])
                                        {
                                                Q.push(k);
                                                done[k] = true;
                                ...
                                {
                                        range[num] = std::max(range[num], HG[k * num_of_agents +
j]); if (!done[k])
                                        {
                                                Q.push(k);
                                                done[k] = true;
                ...
                {
                        rst += std::max(HG[indices[0] * num_of_agents + indices[1]], HG[indices[1] *
num_of_agents + indices[0]]); // add edge weight continue;
                }
                ...
                for (int j = 0; j < num; j++)
                {
                        ...
                        {
                                G[j * num + k] = std::max(HG[indices[j] * num_of_agents +
indices[k]], HG[indices[k] * num_of_agents + indices[j]]);
                        }
                }
...
bool CBSHeuristic::buildDependenceGraph(CBSNode& node, vector<int>& CG, int& num_of_CGedges) {
  ...
  for (auto& conflict : node.conflicts) {
    ...
    if (CG[a1 * num_of_agents + a2] > 0) continue;
    if (conflict->priority == conflict_priority::CARDINAL) {
    ...
    if (CG[idx]) {
      num_of_CGedges++;
      conflict->priority =
          conflict_priority::PSEUDO_CARDINAL;  // the two agents are dependent, although resolving
                                               // this conflict might not increase the cost
    }
  }
  runtime_build_dependency_graph += (double)(clock() - start_time) / CLOCKS_PER_SEC;
  return true;
...
bool CBSHeuristic::buildWeightedDependencyGraph(CBSNode& node, vector<int>& CG) {
  for (const auto& conflict : node.conflicts) {
    ...
    if (got != lookupTable[a1][a2].end())  // check the lookup table first
    {
      ...
      if (!cardinal &&
          !mutex_reasoning)  // using merging MDD methods before runing 2-agent instance
      {
      ...
      if (cardinal)  // run 2-agent solver only for dependent agents
      {
        ...
        lookupTable[a1][a2][HTableEntry(a1, a2, &node)] =
            make_tuple(0, 1, 0);  // h=0, #CT nodes = 1
        CG[idx] = 0;
...
bool CBSHeuristic::buildWeightedDependencyGraph(ECBSNode& node, const vector<int>& min_f_vals,
...
pair<int, int> CBSHeuristic::solve2Agents(int a1, int a2, const CBSNode& node, bool cardinal) {
  ...
  if (cardinal) lowerbound += 1;
  cbs.solve(time_limit - runtime, lowerbound, upperbound);
  num_solve_2agent_problems++;

  pair<int, int> rst;
  if (cbs.runtime >= time_limit - runtime ||
      cbs.num_HL_expanded > node_limit)        // time out or node out
...
tuple<int, int, int> CBSHeuristic::solve2Agents(int a1, int a2, const ECBSNode& node) {
  ...
  if (cbs.runtime >= time_limit - runtime)    // time out
    ...
    return make_tuple(cbs.getLowerBound() - cbs.dummy_start->g_val, cbs.getInitialPathLength(0),
    ...
    return make_tuple(cbs.solution_cost - cbs.dummy_start->g_val, cbs.getInitialPathLength(0),
                      cbs.getInitialPathLength(1));
}
...
: ";
        ...
        if (type == heuristics_type::WDG && rst > 0)
        {
                ...
                cbs.setDisjointSplitting(disjoint_splitting);
                cbs.setBypass(false); // I guess that bypassing does not help two-agent path
finding??? cbs.setRectangleReasoning(rectangle_reasoning);
                cbs.setCorridorReasoning(corridor_reasoning);
                cbs.setTargetReasoning(target_reasoning);
                cbs.setMutexReasoning(mutex_reasoning);
                ...
                if (cbs.runtime >= time_limit - runtime) // time out
                        rst = (int)cbs.min_f_val - cost_shortestPath; // using lowerbound to
approximate else if (cbs.solution_cost  < 0) // no solution rst = cbs.solution_cost; else rst =
cbs.solution_cost - cost_shortestPath; num_solve_2agent_problems++;
        }
  ...
  for (int i = 0; i < num_of_agents; i++) {
    ...
    for (int j = 0; j < (int)indices.size(); j++) {
      for (int k = j + 1; k < (int)indices.size(); k++) {
        ...
        if (subgraph[j * indices.size() + k] > 0) num_edges++;
      }
    }
    ...
    if (num_edges > ILP_edge_threshold) {
      ...
      double runtime = (double)(clock() - start_time) / CLOCKS_PER_SEC;
      if (runtime > time_limit) return -1;  // run out of time
    } else {
...
int CBSHeuristic::minimumVertexCover(const std::vector<int>& CG, int old_mvc, int cols,
                                     int num_of_CGedges) {
  clock_t t = clock();
  int rst = 0;
  if (num_of_CGedges < 2) return num_of_CGedges;
...
bool CBSHeuristic::KVertexCover(const std::vector<int>& CG, int num_of_CGnodes, int num_of_CGedges,
                                int k, int cols) {
  double runtime = (double)(clock() - start_time) / CLOCKS_PER_SEC;
  if (runtime > time_limit) return true;  // run out of time
  if (num_of_CGedges == 0)
    return true;
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (KVertexCover(CG_copy, num_of_CGnodes - 1, num_of_CGedges_copy, k - 1, cols)) return true;
  }
  return false;
}
...
int CBSHeuristic::DPForWMVC(std::vector<int>& x, int i, int sum, const std::vector<int>& CG,
  ...
  if (sum >= best_so_far) return MAX_COST;
  double runtime = (double)(clock() - start_time) / CLOCKS_PER_SEC;
  if (runtime > time_limit)
    return -1;  // run out of time
...
/*int CBSHeuristic::ILPForConstrainedWMVC(const std::vector<int>& CG, const std::vector<int>&
node_weights)
{
...
// recusive component of weighted vertex cover
int CBSHeuristic::DPForConstrainedWMVC(std::vector<bool>& x, int i, int sum,
                                       const std::vector<int>& CG, const std::vector<int>& range,
                                       int& best_so_far) {
  if (sum >= best_so_far) return INT_MAX;
  double runtime = (double)(clock() - start_time) / CLOCKS_PER_SEC;
  if (runtime > time_limit)
    return -1;  // run out of time
  else if (i == (int)x.size()) {
...
bool CBSHeuristic::dependent(int a1, int a2,
                             HLNode& node)  // return true if the two agents are dependent
{
...
bool CBSHeuristic::SyncMDDs(const MDD& mdd, const MDD& other)  // assume mdd.levels <= other.levels
{
  ...
  // Cheaply find the coexisting nodes on level zero - all nodes coexist because agent starting
  ...
  for (size_t i = 1; i < copy.levels.size(); i++) {
    for (auto node = copy.levels[i].begin(); node != copy.levels[i].end();) {
      // Go over all the node's parents and test their coexisting nodes' children for co-existance
      // with this node
        ...
        for (const MDDNode* parentCoexistingNode : (*parent)->coexistingNodesFromOtherMdds) {
          for (const MDDNode* childOfParentCoexistingNode : parentCoexistingNode->children) {
            ...
            for (; it != (*node)->coexistingNodesFromOtherMdds.cend(); ++it) {
              if (*it == childOfParentCoexistingNode) break;
            }

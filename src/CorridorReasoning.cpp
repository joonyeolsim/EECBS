...
{
  if (paths[conflict->a1]->size() <= 1 || paths[conflict->a2]->size() <= 1) return 0;
  ...
  endpoints_time[0] = getExitingTime(*paths[conflict->a1], t);
  ;  // the first timestep when agent 1 exits the corridor
  endpoints_time[1] = getExitingTime(*paths[conflict->a2], t);
  ;  // the first timestep when agent 2 exits the corridor
  endpoints[0] =
      paths[conflict->a1]->at(endpoints_time[0]).location;  // the exit location for agent 1
  endpoints[1] =
      paths[conflict->a2]->at(endpoints_time[1]).location;  // the exit location for agent 2
...
shared_ptr<Conflict> CorridorReasoning::findCorridorConflict(const shared_ptr<Conflict>& conflict,
  ...
  int t[2];
  for (int i = 0; i < 2; i++) t[i] = getEnteringTime(*paths[a[i]], *paths[a[1 - i]], timestep);
  if (t[0] > t[1]) {
    int temp = t[0];
    t[0] = t[1];
    t[1] = temp;
  ...
  int u[2];
  for (int i = 0; i < 2; i++) u[i] = paths[a[i]]->at(t[i]).location;
  if (u[0] == u[1]) return nullptr;
  for (int i = 0; i < 2; i++) {
    bool found = false;
    for (int time = t[i]; time < (int)paths[a[i]]->size() && !found; time++) {
      if (paths[a[i]]->at(time).location == u[1 - i]) found = true;
    }
  ...
  t3_ = search_engines[conflict->a1]->getTravelTime(paths[conflict->a1]->front().location, u[1],
                                                    ct1, t3 + 2 * corridor_length + 1);
  ...
  t4_ = search_engines[conflict->a2]->getTravelTime(paths[conflict->a2]->front().location, u[0],
                                                    ct2, t3 + corridor_length + 1);
...
num_col, int map_size)
{
        int length = INT_MAX;
        // generate a heap that can save nodes (and a open_handle)
        boost::heap::pairing_heap< AStarNode*, boost::heap::compare<AStarNode::compare_node> > heap;
        // boost::heap::pairing_heap< AStarNode*, boost::heap::compare<AStarNode::compare_node>
>::handle_type open_handle; unordered_set<AStarNode*, AStarNode::NodeHasher, AStarNode::eqnode>
nodes;
                ...
                for (int direction = 0; direction < 4; direction++)
                {
                        ...
                        {  // if that grid is not blocked
                                if ((curr->loc == blocked.first && next_loc == blocked.second) ||
                                        (curr->loc == blocked.second && next_loc == blocked.first))
// use the prohibited edge
                                {
                                        continue;
                                }
                                int next_g_val = curr->g_val + 1;
                                auto next = new LLNode(next_loc, next_g_val,
getManhattanDistance(next_loc, end, num_col), nullptr, 0); auto it = nodes.find(next); if (it ==
nodes.end()) {  // add the newly generated node to heap and hash table next->open_handle =
heap.push(next); nodes.insert(next);
                                }
                                else {  // update existing node's g_val if needed (only in the heap)
                                        delete(next);  // not needed anymore -- we already generated
it before auto existing_next = *it; open_handle = (*it)->open_handle; if (existing_next->g_val >
next_g_val)
                                        {
...
/*
// run space-time A* to find the length of the shortest path between start and goal without using
the blocked edge from either direction.
        ...
        const ConstraintTable& constraint_table, int upper_bound)
{
        ...
        // boost::heap::pairing_heap< AStarNode*, boost::heap::compare<LLNode::compare_node>
>::handle_type open_handle; unordered_set<AStarNode*, AStarNode::NodeHasher, AStarNode::eqnode>
nodes; auto root = new AStarNode(start, 0, instance.getManhattanDistance(start, end), nullptr, 0);
        ...
        while (!open_list.empty())
        {
                ...
                for (int next_location : next_locations)
                {
                        ...
                        if ((curr->location == blocked.first && next_location == blocked.second) ||
                                (curr->location == blocked.second && next_location ==
blocked.first)) // use the prohibited edge
                        {
                                continue;
                        }
                        if (!constraint_table.constrained(next_location, next_timestep) &&
                                !constraint_table.constrained(curr->location, next_location,
next_timestep)) {  // if that grid is not blocked int next_h_val =
instance.getManhattanDistance(next_location, end); if (next_g_val + next_h_val >= upper_bound) //
the cost of the path is larger than the upper bound continue; auto next = new
AStarNode(next_location, next_g_val, next_h_val, nullptr, next_timestep); auto it =
nodes.find(next); if (it == nodes.end()) {  // add the newly generated node to heap and hash table
                                        next->open_handle = open_list.push(next);
                                        nodes.insert(next);
                                }
                                else {  // update existing node's g_val if needed (only in the heap)
                                        delete(next);  // not needed anymore -- we already generated
it before auto existing_next = *it; if (existing_next->g_val > next_g_val)
                                        {
                                                existing_next->g_val = next_g_val;
        ...
        ReservationTable& reservation_table, int upper_bound)
{
        ...
        // boost::heap::pairing_heap< AStarNode*, boost::heap::compare<LLNode::compare_node>
>::handle_type open_handle; unordered_set<SIPPNode*, SIPPNode::NodeHasher, SIPPNode::eqnode> nodes;

        Interval interval = reservation_table.get_first_safe_interval(start);
        assert(get<0>(interval) == 0);
        auto root = new SIPPNode(start, 0, instance.getManhattanDistance(start, end), nullptr, 0,
interval); root->open_handle = open_list.push(root);  // add root to heap nodes.insert(root); // add
root to hash_table (nodes)
...
get<1>(curr->interval) + 1))
                        {
                                ...
                                if (next_g_val + next_h_val >= upper_bound) // the cost of the path
is larger than the upper bound continue; auto next = new SIPPNode(next_location, next_g_val,
next_h_val, nullptr, next_timestep, interval); auto it = nodes.find(next); if (it == nodes.end()) {
// add the newly generated node to heap and hash table next->open_handle = open_list.push(next);
                                        nodes.insert(next);
                                }
                                else {  // update existing node's g_val if needed (only in the heap)
                                        delete(next);  // not needed anymore -- we already generated

...
// takes the paths_found_initially and UPDATE all (constrained) paths found for agents from curr to
// start
...
void CBS::findConflicts(HLNode& curr, int a1, int a2) {
  int min_path_length =
      (int)(paths[a1]->size() < paths[a2]->size() ? paths[a1]->size() : paths[a2]->size());
  ...
  switch (conflict_selection_rule) {
      ...
      switch (conflict.type) {
        ...
        case conflict_type::CORRIDOR:
          conflict.secondary_priority =
              min(get<2>(conflict.constraint1.front()), get<3>(conflict.constraint1.front()));
          break;
        default:
          break;
      }
    ...
    case conflict_selection::CONFLICTS:
      for (const auto& c : node.conflicts) {
        if (c->a1 == conflict.a1 || c->a2 == conflict.a1) count[0]++;
        if (c->a1 == conflict.a2 || c->a2 == conflict.a2) count[1]++;
      }
...
void CBS::computeConflictPriority(shared_ptr<Conflict>& con, CBSNode& node) {
  ...
  {
    if (!cardinal1) cardinal1 = mdd1->levels[timestep].size() == 1;
    if (!cardinal2) cardinal2 = mdd2->levels[timestep].size() == 1;
  }
  ...
  while (!node.unknownConf.empty()) {
    ...
    if (rectangle_reasoning && (int)paths[con->a1]->size() > timestep &&
        (int)paths[con->a2]->size() >
            timestep &&  // conflict happens before both agents reach their goal locations
...
bool CBS::findPathForSingleAgent(CBSNode* node, int ag, int lowerbound) {
  ...
  // find a path
  Path new_path =
      search_engines[ag]->findOptimalPath(*node, initial_constraints[ag], paths, ag, lowerbound);
          ...
          for (int ag = 0; ag < num_of_agents; ag++)
          {
                  ...
                  for (int i = t; i < (int)paths[ag]->size(); i++)
                  {
                          if (paths[ag]->at(i).location == x)
                          {
                                  ...
                                  {
                                          runtime_generate_child += (double)(clock() - t1) /
      ...
      if (cleanup_list.top()->getFVal() > cost_lowerbound) {
        if (screen == 3) {
          cout << "  Note -- FOCAL UPDATE!! from |FOCAL|=" << focal_list.size()
               << " with |OPEN|=" << cleanup_list.size() << " to |FOCAL|=";
        }
...
set<int> CBS::getInvalidAgents(
    const list<Constraint>& constraints)  // return agents that violate the constraints
{
  set<int> agents;
  int agent, x, y, t;
  constraint_type type;
...
void CBS::saveResults(const string& fileName, const string& instanceName) const {
  ...
  if (!exist) {
    ...
    addHeads << "runtime,#high-level expanded,#high-level generated,#low-level expanded,#low-level "
                "generated,"
             << "solution cost,min f value,root g value, root f value,"
             << "#adopt bypasses,"
             << "cardinal conflicts,"
             << "standard conflicts,rectangle conflicts,corridor conflicts,target conflicts,mutex "
                "conflicts,"
             ...
             << "runtime of rectangle conflicts,runtime of corridor conflicts,runtime of mutex "
                "conflicts,"
             << "runtime of building MDDs,runtime of building constraint tables,runtime of "
                "building CATs,"
  ...
  stats << runtime << "," << num_HL_expanded << "," << num_HL_generated << "," << num_LL_expanded
        << "," << num_LL_generated << "," <<

      solution_cost << "," << cost_lowerbound << "," << dummy_start->g_val << ","
        << dummy_start->g_val + dummy_start->h_val << "," <<

      num_adopt_bypass << "," << num_cardinal_conflicts << "," << num_standard_conflicts << ","
        << num_rectangle_conflicts << "," << num_corridor_conflicts << "," << num_target_conflicts
        << "," << num_mutex_conflicts << "," <<

      num_cleanup << "," << num_open << "," << num_focal << "," <<

      heuristic_helper.num_solve_MVC << "," << heuristic_helper.num_merge_MDDs << ","
        << heuristic_helper.num_solve_2agent_problems << "," << heuristic_helper.num_memoization
      ...
      runtime_detect_conflicts << "," << rectangle_helper.accumulated_runtime << ","
        << corridor_helper.accumulated_runtime << "," << mutex_helper.accumulated_runtime << ","
...
void CBS::saveStats(const string& fileName, const string& instanceName) {
  ...
  {
                  stats << get<0>(ins) << "," << get<1>(ins) << "," << get<2>(ins)->time_generated
  << "," << get<3>(ins) << "," << get<4>(ins) << endl;
  }
  stats.close();*/
  /*if (solution_found)
  {
...
void CBS::saveCT(const string& fileName) const  // write the CT to a file
{
  // Write the tree graph in dot language to a file
  {
    ...
    for (const auto& node : allNodes_table) {
      ...
      if (path_to_goal.find(node) == path_to_goal.end()) {
        ...
        output << node->parent->time_generated << " -> " << node->time_generated << " [color=red]"
               << endl;
      }
    }
    output << "}" << endl;
  ...
  // Write the stats of the tree to a CSV file
  {
    ...
    // header
    output << "time generated,g value,h value,h^ value,d value,depth,time expanded,chosen from,h "
              "computed,"
    ...
    for (auto& node : allNodes_table) {
      output << node->time_generated << "," << node->g_val << "," << node->h_val << ","
             << node->getFHatVal() - node->g_val << "," << node->distance_to_go << ","
             << node->depth << "," << node->time_expanded << "," << node->chosen_from << ","
             << node->h_computed << "," << node->f_of_best_in_cleanup << ","
             << node->f_hat_of_best_in_cleanup << "," << node->d_of_best_in_cleanup << ","
             << node->f_of_best_in_open << "," << node->f_hat_of_best_in_open << ","
             << node->d_of_best_in_open << "," << node->f_of_best_in_focal << ","
             << node->f_hat_of_best_in_focal << "," << node->d_of_best_in_focal << ",";
...
string CBS::getSolverName() const {
  ...
  if (rectangle_reasoning) name += "+R";
  if (corridor_reasoning) name += "+C";
  if (target_reasoning) name += "+T";
  if (mutex_reasoning) name += "+MP";
  if (bypass) name += "+BP";
    ...
    while (foundBypass) {
      ...
      if (foundBypass) {
        ...
        if (curr->chosen_from == "cleanup")
          ...
          num_focal++;
        if (curr->conflict->priority == conflict_priority::CARDINAL) num_cardinal_conflicts++;
        curr->clear();
      }
    }
  }  // end of while loop
...
CBS::CBS(vector<SingleAgentSolver*>& search_engines,
         const vector<ConstraintTable>& initial_constraints, vector<Path>& paths_found_initially,
         int screen)
    : screen(screen),
      suboptimality(1),
  ...
  if (paths_found_initially.empty()) {
    ...
    for (auto i : agents) {
      ...
      paths_found_initially[i] =
          search_engines[i]->findOptimalPath(*root, initial_constraints[i], paths, i, 0);
      if (paths_found_initially[i].empty()) {
        if (screen >= 2) cout << "No path exists for agent " << i << endl;
        delete root;
        return false;
      }
  ...
  for (int a1 = 0; a1 < num_of_agents; a1++) {
    ...
    for (int a2 = a1 + 1; a2 < num_of_agents; a2++) {
      size_t min_path_length =
          paths[a1]->size() < paths[a2]->size() ? paths[a1]->size() : paths[a2]->size();
      for (size_t timestep = 0; timestep < min_path_length; timestep++) {
        ...
        if (loc1 == loc2) {
          ...
          return false;
                   ...
                   loc2 == paths[a1]->at(timestep + 1).location) {
          cout << "Agents " << a1 << " and " << a2 << " collides at (" << loc1 << "-->" << loc2
